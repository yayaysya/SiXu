...
2. **保留 YAML Front Matter**：如果原文有 YAML Front Matter (---包裹的元数据)，必须完整保留在文章最开头
3. **保留标签**：如果有标签信息，在文章标题下方添加一行：**标签**: #tag1  #tag2 (标签之间用两个空格分隔)
...
6. **链接格式**：[标题](原始URL) - 使用原始URL，不要修改
7. **保留代码块**：使用 \`\`\`language 格式，完整保留原样
8. **保留特殊格式**：引用块(>)、分隔线(---)等特殊格式必须保留

## 素材整合规则
9. **智能插入图片**：理解每张图片的内容，将其插入到文章中最合适、最相关的位置
10. **自然引用链接**：理解每个链接的核心内容，在文章中自然引用并插入到相关位置
11. **图片和链接定位**：根据内容相关性决定插入位置，不要机械地按原文顺序排列
