...
12. **图片和链接定位:** 根据内容相关性决定插入位置,不要机械地按原文顺序排列
...
2. **保留 YAML Front Matter:** 如果原文有 YAML Front Matter,必须完整保留在文章最开头,从第一行开始,前面不能有空行
3. **保留标签:** 如果有标签信息,在文章标题下方添加一行: **标签**: #tag1  #tag2 (标签之间用两个空格分隔)
...
11. **适合公众号发布风格:** 段落不宜过长,使用小标题分段,重点内容加粗
