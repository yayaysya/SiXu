...
- **学习数据看板**：进度追踪、活跃度可视化
- **学习路径功能**：结构化学习流程创建和管理
- **个人资料界面**：完整的用户个人中心，包含基本信息、成就系统、生涯总览和管理中心
...
│   ├── components/        # 可复用组件
│   │   └── profile/       # 个人资料界面组件
│   │       ├── UserProfileView.ts      # 主界面组件
│   │       ├── BasicInfoCard.ts       # 基本信息卡片
│   │       ├── AchievementCard.ts     # 勋章系统卡片
│   │       ├── StatsOverviewCard.ts   # 生涯总览卡片
│   │       ├── ManagementCenter.ts    # 管理中心
│   │       └── modals/               # 弹窗组件
│   │           ├── HelpModal.ts       # 帮助反馈弹窗
│   │           └── AboutModal.ts      # 关于我们弹窗
│   ├── flashcard/         # 闪卡系统完整实现
...
- **关键文件**：
  ...
  - `src/views/combineView.ts`：主要用户界面，集成个人资料页面
  - `src/components/profile/UserProfileView.ts`：个人资料界面主组件
...
- **组件化个人资料界面**：采用模块化组件设计，便于维护和扩展
- **响应式设计优先**：支持 320px 到宽屏的全尺寸范围适配
- **页面布局一致性**：个人资料页面与学习页面保持统一的视觉风格

## 7. 注意事项
- **移动端适配**：闪卡系统和个人资料界面都已完成移动端优化
- **并发处理**：任务队列系统支持并发 AI 请求，避免阻塞 UI
- **错误处理**：完善的错误处理和用户反馈机制
- **配置管理**：通过 Obsidian 设置界面管理所有配置
- **文件命名**：使用 `{name}_{type}_{date}.md` 格式避免冲突
- **CSS 架构**：个人资料界面采用 BEM 式命名规范，保持样式可维护性
- **组件生命周期**：个人资料组件支持动态数据更新和刷新
